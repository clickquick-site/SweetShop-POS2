...
function saveSettingsApp() {
  ...
  const ff=document.getElementById("sFontFamily");
  if(ff) DB.settings.fontFamily=ff.value;
...
function previewFont() {
  ...
  if(f) document.body.style.fontFamily = `'${f}', system-ui, sans-serif`;
}
...
function saveItem(){
  ...
  const costPrice = parseFloat(costEl.value);
  const qty       = parseInt(qtyEl.value);
  const exp       = expEl.value;

  // FIX: validation améliorée
  ...
  if (!barcode) { showToast("أدخل الباركود","error"); barcodeEl.focus(); return; }
  if (isNaN(price) || price < 0)     { showToast("أدخل سعر البيع","error"); priceEl.focus(); return; }
  if (isNaN(costPrice) || costPrice < 0) { showToast("أدخل سعر الشراء","error"); costEl.focus(); return; }
  if (isNaN(qty) || qty < 0)         { showToast("أدخل الكمية","error"); qtyEl.focus(); return; }
...
function editUser(index){
  ...
  const newName=window.prompt(t("edit_btn")+" "+t("col_name")+":",user.name)||user.name;
  const newPin=window.prompt("PIN:",user.pin)||user.pin;
  const newRole=window.prompt("manager/baker:",user.role)||user.role;
  if (newPin.length!==4||!/^\d+$/.test(newPin)){ showToast(t("msg_pin_4"),"error"); return; }
