...
function saveItem(){
  ...
  const brand     = brandEl.value.trim(); // ✅ تحسين: قبول الماركة النصية مباشرة
  ...
  if (!brand)   { showToast("أدخل الماركة أولاً","error"); brandEl.focus(); return; }

  if (!barcode) {
    barcode = "AUTO-" + Date.now().toString(36).toUpperCase() + "-" + Math.random().toString(36).slice(2,5).toUpperCase();
    barcodeEl.value = barcode;
    // ✅ إصلاح Bug #7: تنبيه واضح للمستخدم بأن الباركود تلقائي ولا يعمل مع السكانر
    showToast("⚠️ باركود تلقائي — لن يعمل مع قارئ الباركود. يُنصح بإدخال باركود حقيقي.", "warning");
  }
  ...
  if (existing) {
    // ✅ إصلاح Bug #9: تحذير عند تكرار الباركود لمنتج مختلف
    ...
    // ✅ تحسين: إضافة الماركة تلقائياً إذا لم تكن موجودة
    let fam = DB.families.find(f => f.name === type);
    let brand_obj = DB.brands.find(b => b.name === brand && b.familyId === (fam ? fam.id : null));
    if (!brand_obj && fam) {
      DB.brands.push({ id: uid(), name: brand, familyId: fam.id });
    }
...
function renderStock(){
  ...
  // ✅ تحسين: البحث الشامل - يبحث في العائلة والماركة والحجم والباركود
  const list=q?DB.stock.filter(i=>i.type.toLowerCase().includes(q)||i.brand.toLowerCase().includes(q)||(i.size&&i.size.toLowerCase().includes(q))||i.barcode.includes(q)):DB.stock;
...
function searchSuggestions() {
  ...
  // ✅ تحسين: البحث الشامل - يبحث في الاسم والماركة والعائلة والباركود والحجم
